...
int main(int argc, char **argv) {
	...
	if(error_id != cudaSuccess) {
		printf("cuda device count returned %d ==> %s\n", (int)error_id, cudaGetErrorString(error_id));
		printf("Result = FAILED\n");
		exit(EXIT_FAILURE);
	}

	if(deviceCount = 0) {
		printf("Their is no available device that supports CUDA\n");
	} else {
	...
	cudaRuntimeGetVersion(&runtimeVerstion);
	...
	printf("Max size of each dimenstion per block %d x %d x %d \n", deviceProp.maxThreadsDim[0], deviceProp.maxThreadsDim[1], deviceProp.maxThreadsDim[2]);

	printf("Max size of each dimension of grid %d x %d x %d\n", deviceProp.maxGridSize[0], deviceProp.maxGridSize[1], deviceProp.maxGridSize[2]);
